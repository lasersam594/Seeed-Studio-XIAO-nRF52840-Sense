/*
  Seeed Studio XIAO nRF52840 Sense Sensor Test using External LEDs V5.

  This is intended for use with laser-etched glass sphere solar system (and other) trinkets.
  ...
  port.  The required libraries are all either built into the Arduino IDE or Arduino Cloud Editor, or readily found via a Web
  search.  The primary difference between the Rev1 and Rev2 sketches are the libraries for the IMU and T/H.
  ...
  In addition, for the original version, the on-board USER LED (the CHARGE led) and RGB LEDs provide visual output:
  ...
  1. Gyroscope: Displays the values for roll, pitch, and yaw as the brightness of each if the RGB leds as roll (+red/-cyan),
     ...
     and yaw offsets.  If GyroAutoCal is enabled, the board must remain stationary at startup while the RGB LEDs are blinking.
     The default duration is ~12 blinks, under 1 second.
  ...
  Like the built-in LEDs, it also assumes common anode LEDs so full off is 255 and full on is 0.  It can drive a common-anode
  RGB LED directly using the VUSB (+5 V) and a 120 ohm current limiting resistor in series with a 1N4148 diode to assure that
  ...
  digital pins.  This scheme was tested using an RGB common-anode LED probably a closeout from MPPA.  With external drivers,
...
#define Version 5 // Same functionality as the version using the built-in RGB LEDs
