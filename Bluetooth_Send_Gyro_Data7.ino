...
void SendGyroData(BLEDevice peripheral) {
  ...
  if (!Sequence_Number) {
    if (sendData == 1) Serial.println("Peripheral does not have SN Characteristic!");
    peripheral.disconnect();
    return;
  }
  else if (sendData == 1) Serial.println("Peripheral has SN Characteristic!");
