...
void setup() {
  ...
  if (data1 == 1) Serial.println("BLE Gyro Monitor Peripheral");
}
...
// Connection active
    ...
    while (central.connected()) {
      ...
      if (Sequence_Number.written()) SN = Sequence_Number.value();
       if (SN != SNprev) {
      if (Gyro_Roll.written())  
      grint = Gyro_Roll.value(); grint -= 10000; // Kludge to get signed numbers through Bluetooth
      if (Gyro_Pitch.written()) 
      gpint = Gyro_Pitch.value(); gpint -= 10000;
      if (Gyro_Yaw.written())   
      gyint = Gyro_Yaw.value(); gyint -= 10000;
