...
uint8_t RGB_LEDColor = 0;  // 8 bits for color value
...
void setup() {
  ...
  // Set up LED used for connect statuserererer
...
void SendGyroData(BLEDevice peripheral) {
  ...
  while (peripheral.connected()) {
    ...
    // Kludge to get singed numbers through Bluetooth
    grint += 10000;
    gpint += 10000;
    gyint += 10000;
...
void RGB_LED_Cycle_Colors() {
  RGB_LED_Color(BLACK, 0); delay(50);
  RGB_LED_Color(GRAY, 1.0); delay(50);
  RGB_LED_Color(MAGENTA, 0.2); delay(50);
  RGB_LED_Color(BLUE, 0.1); delay(50);
  RGB_LED_Color(CYAN, 0.2); delay(50);
  RGB_LED_Color(GREEN, 1.0); delay(50);
  RGB_LED_Color(YELLOW, 0.5); delay(50);
  RGB_LED_Color(ORANGE, 0.8); delay(50);
  RGB_LED_Color(RED, 1.0); delay(50);
  RGB_LED_Color(ORANGE, 0.8); delay(50);
  RGB_LED_Color(YELLOW, 0.5); delay(50);
  RGB_LED_Color(GREEN, 1.0); delay(50);
  RGB_LED_Color(CYAN, 0.2); delay(50);
...
void RGB_Gyro_Colors(int roll, int pitch, int yaw, float atten) {
  ...
  ledg = 0;
  ledb = 0;
  if ((fabs(roll) > 2) || (fabs(pitch) > 2) || (fabs(yaw) > 2)) {  // Update if above threahold
